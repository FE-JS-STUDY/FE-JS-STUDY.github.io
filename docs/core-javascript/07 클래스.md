...
하위 클래스의 메서드에서 상위 클래스의 메서드 실행 결과를 바탕으로 추가적인 적업을 수해하고 싶을 때 매번 이런식으로 해결하는 것은 가독성이 떨어진다.
...
var Square = class extends Rectangle {		//	extends Rectangle 을 통해 Square을 SubClass로 만들 수 있다
  constructor(width) {
